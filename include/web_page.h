...
<p><span class="chip" id="home">Home</span> <span class="chip" id="target">At Target</span> <span class="chip" id="fault">Fault</span> <span class="chip" id="refused">Refused</span> <span class="chip warn" id="manual" hidden>Manual</span></p>
<table><tr><td>CPU2 State</td><td id="state">-</td></tr><tr><td>PLC Pattern</td><td id="pat">-</td></tr><tr><td>PLC Speed</td><td id="spd">-</td></tr><tr><td>Ticker</td><td id="ticker">-</td></tr><tr><td>Input Glitches</td><td id="gl">-</td></tr><tr><td>Fault Type</td><td id="ft">-</td></tr><tr><td>Manual Jog</td><td id="mj">-</td></tr><tr><td>DIP Switches (Raw)</td><td id="dp">-</td></tr><tr><td>Refused Reason</td><td id="rr">-</td></tr><tr><td>CPU2 I2C Errors</td><td id="i2c">-</td></tr>
...
function render(s){
 ...
 const c=s.cpu2;$('c2').textContent=c.ok?'Online':'Offline';chip('c2',c.ok,c.ok?'on':'bad');
 chip('home',s.home);chip('target',s.target);chip('fault',s.fault,'bad');chip('refused',s.refused,'bad');const m=s.manual,req=m.dip||m.soft||m.active;$('manual').hidden=!req;if(req){$('manual').textContent='Manual ('+(m.dip&&m.soft?'Switch + PLC':m.dip?'Switch':'PLC')+')'+(m.active?'':' - Waiting');}$('dp').textContent='DIP 1 (IP bit 0)='+(m.dips&1)+'   DIP 2 (IP bit 1)='+(m.dips>>1&1)+'   DIP 3 (manual)='+(m.dips>>2&1);$('mj').textContent=m.active?('Spreader '+m.spr+', '+(m.open?'Opening':m.close?'Closing':'Stopped')):'-';
 $('state').textContent=c.ok?STATE[c.state]||c.state:'-';$('pat').textContent=s.pattern+(s.pattern?'':' (home)');$('spd').textContent=s.speed;$('ticker').textContent=s.tick;$('gl').textContent=s.glitches;$('ft').textContent=FTYPE[s.ftype]||s.ftype;$('rr').textContent=s.refused?(REASON[s.rreason]||s.rreason):'None';$('i2c').textContent=c.ok?(c.ioFails+c.otherFails)+' ('+c.ioFails+' of '+c.ioReads.toLocaleString()+' sensor reads)':'-';
