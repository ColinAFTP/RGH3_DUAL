...
function render(s){
 ...
 chip('home',s.home);chip('target',s.target);chip('fault',s.fault,'bad');chip('refused',s.refused,'bad');const m=s.manual,req=m.dip||m.soft||m.active;$('manual').hidden=!req;if(req){$('manual').textContent='Manual ('+(m.dip&&m.soft?'Switch + PLC':m.dip?'Switch':'PLC')+')'+(m.active?'':' - Waiting');}$('dp').textContent='SW1='+(m.dips&1)+'  SW2='+(m.dips>>1&1)+'  SW3 (manual)='+(m.dips>>2&1);$('mj').textContent=m.active?('Spreader '+m.spr+', '+(m.open?'Opening':m.close?'Closing':'Stopped')):'-';
