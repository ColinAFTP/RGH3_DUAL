...
## Manual mode: complete bench results (2026-09-20)

All PASSED on the desk board (no motors, proxies simulated by switches):
- Modbus jog test (`tools/modbus_manual_test.js`): enter via coil 104; open S8 pushes S9 and S10 (all +41.4 mm, S6/S7 still); close with sensors on = no movement; dead man stops ~1 s after the PLC goes silent; spreader 5 refused; pattern request refused with reason 8; leaving manual = automatic home with coil 101 on.
- DIP switch (PCB DIP 3): ON = manual mode; OFF = automatic home. It stops a running TeensyStep move with deceleration ("move or homing stopped by the manual DIP switch"), At Target stays off, positions stay known and the automatic home runs from where it stopped (speed = PLC speed register). DIP + PLC request in parallel: manual stays on until BOTH are off (DIP off with the PLC request on = still manual; clearing the PLC request then homes).
- Power-up with the DIP switch on and proxy 2 off: comes up in manual mode, positions unknown, NO automatic home. Leaving manual then did a search home ("search, pulses only").
- Closing jog with a real sensor change (`tools/modbus_close_test.js`): switching proxy 2 ON stopped the jog 7 ms later ("touching its neighbour"); no further movement.
- Jog length guard: one jog stops after `MANUAL_MAX_JOG_STEPS` (16,500 pulses, ~11 s at 1500 steps/s): log "jog stopped: travel limit", and the coil must be released before a new jog (latched).
...
## The "mystery Modbus client" was a CPU1 bug (found and fixed 2026-09-20)

Symptom: the event log showed "PLC connected / PLC disconnected" every ~2.5 s for minutes at a time, starting right after one of my test scripts disconnected, although nothing was connecting (Colin alone in the office, simulator off; Colin views the dashboard in Edge on the HOST PC, which the VM's scripts reach as 192.168.2.7; the VM itself is 192.168.2.10).
Root cause: the web server (port 80) and the Modbus server (port 502) share the network stack's small socket pool. `ethernetClient` (the Modbus client object) kept pointing at the socket number of a Modbus client that had left. When the browser's dashboard request reused that socket number, `ethernetClient.connected()` became true for the length of the web request and false when it closed: phantom "connect/disconnect" events. Worse, `modbusServer.poll()` reads from `ethernetClient`, so it could swallow bytes of the browser's request, and the reconnect logic could close a browser connection when it dropped the "old" client.
...
## The other Modbus client was Colin's MDBUS simulator in the VM (identified 2026-09-21)
